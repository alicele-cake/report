...
document.getElementById('formContainer').addEventListener('submit', function(event) {
    ...
    if (form.id === 'dateForm') {
        ...
        calculateTotalFruitCountAndCreatePieChart(selectedDate,form.id);
...
function startUpdatingChart(selectedDate,form_id) {
    ...
    updateIntervalId = setInterval(async () => {
        ...
        await calculateTotalFruitCountAndCreatePieChart(selectedDate,form_id);
    }, 60000); // 60000 毫秒 = 1 分钟
}
...
function generateDataTable(labels, dailyCounts) {
    ...
    console.log("fruitTypes:", fruitTypes);
    ...
    fruitTypes.forEach(fruitId => {
        ...
        table += `
                <th scope="col">${labels[fruitId]}</th>
        `;
        // table += `
    ...
    dates.forEach((date, index) => {
        ...
        if (entry) {
            Object.keys(entry).forEach(key => {
                if (key !== 'date') {
                    table += `<td class="${class_name[(index%2)]}">${entry[key]}</td>`;
                    // table += `<td>某個數量</td>`;
                    sum=sum+entry[key];
        ...
        //數量
        table += `<td class="${class_name[(index%2)]}">${sum}</td>`;//總和!!
...
// 计算所有水果的总数并创建饼图
async function calculateTotalFruitCountAndCreatePieChart(selectedDate, form_id) {
    ...
    const dataPoints = Object.values(totalCounts);
...
//     canvas.height = chartDiv.clientWidth; // 或根据你需要调整的高度
            ...
            for (const classId of Object.values(FruitClass)) {
                ...
                const fruitQuery = query(
                    ...
                    querySnapshot.forEach((doc) => {
                        ...
                        if (form_id === 'dateForm') {
                            dailyCounts[classId][PutDate] += data.count;
                            ...
                            console.log(currentDate);
                            const yearMonth = getYearMonth(PutDate);
                            dailyCounts[classId][yearMonth] += data.count;
                            
                        }

                        totalCounts[classId] += data.count;
                    });
